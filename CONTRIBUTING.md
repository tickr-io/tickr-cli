...
`npm run generate-contract --prefix console`. After changing
`console/package-lock.json`, run `just refresh-npm-attribution` and review the
updated license evidence and notices. Do not commit `.env.local`, local runtime
state, logs, credentials, or generated build output.
